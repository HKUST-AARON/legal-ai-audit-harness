...
def _claim_anchor_flags(packet: dict) -> list[str]:
    output_units = packet.get("output_units", [])
    ...
    if any(not _nonempty_text(link.get("unit_id")) or link.get("unit_id") not in units for link in packet.get("output_links", [])):
        return ["source_attribution_gap"]
    return []
...
def _review_gate_flags(scenario: dict, policy: dict, target_rank: int) -> list[str]:
    ...
    if target_rank >= policy["status_rank"]["normative_material_screening_output"] and not gate.get("contestability_channel"):
        flags.append("contestation_failure")
