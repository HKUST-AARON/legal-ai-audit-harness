...
The complete evaluation matrix tests one rule: procedural status follows the legal-material chain. It contains 246 scenario packets with 679 embedded records or outputs and 3,651,560 validation operations across public-output, model-output, invariant, status-lattice, adversarial, ablation, replay, certificate, query, ranking and coding-robustness layers. The design is falsification-oriented. Public-output and holdout layers test whether real upstream outputs carry reconstructable legal-material chains. Model-output and repair layers test whether the same fluent authority recommendations change status when source support, counter-material salience and adoption gates change. Metric, baseline, invariant and lattice layers test whether recall, F1, total score, source binding, review posture or partial gate predicates can reproduce the protocol. Attack, challenge, replay and tamper layers test whether any missing or altered link breaks qualification.
