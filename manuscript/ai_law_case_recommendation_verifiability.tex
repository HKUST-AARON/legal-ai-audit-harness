...
\begin{abstract}
Legal AI case recommendation is procedurally important because it can decide which legal authorities become visible before legal reasons are written. Such outputs are no longer ordinary retrieval, because they structure the materials available for argument; yet they are not legal reasons or legal sources. This article names that missing middle category a \textit{normative material screening output} and argues that its procedural status should depend on auditability rather than model sophistication, autonomy or aggregate accuracy. The article formalizes procedural qualification through output effect, system role and a six-part audit vector covering source and corpus, query and version, authority hierarchy, ranking and counter-materials, contestability and adoption logging. It then implements the model in a provider-agnostic harness and tests it through stress scenarios, public legal-record snapshots, raw model outputs, issue-defined positive controls and issue-level ablations. The resulting rule is simple: no reconstructable source chain, no procedural status; no counter-material pathway, no screening qualification; no adoption record, no decision-support reason.
\end{abstract}
...
\]
Warning flags leave the status unchanged while recording a defect. Downgrade and suspension flags cap external status at reference information unless the missing artefact is restored. Withdrawal flags cap status at $p_0$. This formalization yields three invariants. First, necessary gates are non-compensatory within each claimed status: a missing screening gate cannot be offset by high fluency or high scores elsewhere. Second, failure caps dominate aggregate scores: an output with high retrieval recall is still withdrawn if its sources are unreconstructable or an irreversible external action lacks authorization. Third, upstream retrieval metrics are evidence variables, not status variables; they may inform $K$ or issue-packet construction, but they do not by themselves determine $PS_{\Pi}(o)$.
...
The harness implements the mapping in Section 4 directly. Each scenario declares a claimed status, a human-coded audit vector $S,Q,H,K,T,L$, a jurisdiction profile, optional authority sets, optional upstream-performance metrics, optional evidence-packet fields, optional review-gate fields and expected outcomes. The runner computes the allowed status, counter-authority recall, authority coverage, invalid-authority rate, packet-level evidence fidelity, evidence coverage, source-tag coverage and downgrade or withdrawal disposition. A separate sensitivity command varies the normative and decision thresholds and reports status-distribution changes. The validation layers test rule conformance, artifact integrity and status allocation, not legal merits in the large.

The complete validation matrix contains six layers: ten protocol stress scenarios, 120 embedded public metadata records, 60 ordered public-system output records, ten raw model outputs, three issue-defined positive-control packets and twelve issue-level ablations. All 47 committed scenario files reproduced their pre-specified status outcomes. This is artifact validation, not a field study. Stress scenarios test the decision protocol. Metadata and public-system outputs test source reconstruction. Raw model outputs test the gap between legal fluency and procedural status. Positive-control packets test the conditions under which normative material screening is justified. Ablations test whether specific defects trigger the expected downgrade or suspension.

\begin{table}[htbp]
\centering
\caption{Full artifact validation matrix}
\small
\renewcommand{\arraystretch}{1.1}
