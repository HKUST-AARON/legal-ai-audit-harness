...
Proof-carrying status certificates & 4,182 checks / 3,936 obligations & 4,182/4,182; 3,936/3,936 & Replays proof-carrying status certificates for all 246 packets from scenario hash, policy hash, score gate, role cap, failure cap, metrics, proof obligations and derivation hash. \\
...
The certificate layer makes procedural status proof-carrying. For every packet, the harness records the scenario hash, policy hash, policy thresholds, claimed status, jurisdiction profile, score vector, score-based candidate status, role cap, failure cap, disposition, final status, metric bundle, 16 proof obligations and a derivation hash. Replaying those certificates verified 246/246 proof-carrying status certificates, 4,182/4,182 replay checks and 3,936/3,936 certificate proof obligations. A legal AI output can therefore carry its own procedural-status proof object: the status is not just asserted by the tool, but replayable from the audit artefacts.
