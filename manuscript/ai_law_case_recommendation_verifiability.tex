...
\begin{abstract}
Legal AI case recommendation does more than find law: it allocates visibility over legal authority. When a system ranks cases, omits counter-materials or compresses authorities into a generated answer, its output is no longer ordinary retrieval, but it is still not a legal reason. This article names that missing middle category a \textit{normative material screening output} and argues that procedural status should follow auditability, not model sophistication, autonomy or aggregate accuracy. The article formalizes procedural qualification through two dimensions, output effect and system role, and a six-part audit vector covering source and corpus, query and version, authority hierarchy, ranking and counter-materials, contestability and adoption logging. It implements the model in a provider-agnostic harness with output evidence packets, review gates, downgrade and withdrawal rules, ten stress scenarios, six-jurisdiction metadata validation over 120 records, a public-system output pilot over 60 visible records, three issue-defined gold sets and threshold-sensitivity analysis. The result is a sharper rule: a legal AI output may enter procedure only when the legal-material chain behind it can be reconstructed, contested and attributed; otherwise it remains professional support or reference information, however fluent or accurate it appears.
\end{abstract}
...
\item How can these requirements be operationalized and evaluated across case recommendation, generated-output, public metadata, public-output and issue-defined scenarios?
\end{enumerate}

The resulting framework has two dimensions. The output-effect dimension classifies what the output does in the legal process. The system-role dimension classifies how the system enters that process. The two dimensions are then linked to verifiability, contestability and accountability requirements. The model is intentionally output-first. It does not try to rank vendors, certify legal intelligence or solve all questions of AI liability. It offers a decision protocol for a narrower but sharper question: when may a legal AI output move from internal reference to procedural material, and when must it be downgraded or withdrawn?

This article contributes a concept, a model and an artifact. The concept is normative material screening: the intermediate procedural role of AI-generated authority lists, ranked case sets and legal-material summaries. The model is a two-dimensional procedural-status framework tied to a six-part audit vector and explicit status thresholds. The artifact is an executable audit harness that turns the model into observable indicators, output evidence packets, review gates, downgrade rules, sensitivity analysis, synthetic stress scenarios, public metadata records, public-system output snapshots and manually curated issue gold sets.
...
The point of the category is to keep two claims together. First, case recommendation is not merely a technical search problem, because the output organizes the authorities available for argument. Second, the recommendation system is still not a source of law. The recommended case, statute or award may have authority; the ranking does not. Normative material screening therefore gives AI and Law scholarship a more exact object of evaluation than either ``AI as tool'' or ``AI as decision-maker''.
...
The example condenses the minimum design rule. A case recommendation system that claims procedural use should disclose the source collection, record issue labels and run state, distinguish authority levels and subsequent treatment, display contrary or limiting materials, log human adoption or rejection, and permit contestation of omissions, ranking choices and summary characterizations. Jurisdiction-specific authority rules are parameters of the audit, not the subject of the framework.

\section{Implementation and Evaluation}

To test whether the framework is operational rather than merely classificatory, the rules above were implemented in a lightweight command-line audit harness. The implementation uses local JSON scenario files as the unit of evidence and produces both Markdown and JSON run artefacts. For double-anonymous review, the repository should be supplied through an anonymized companion link; the code consists of a scoring module, a command-line runner, jurisdiction profiles, provider-agnostic evidence-packet fields, ten stress-test scenario files, a six-jurisdiction public metadata validation, a public legal-system output pilot, three issue-defined gold sets, threshold-sensitivity reports and continuous-integration tests.
...
The harness was then applied to ordered outputs from public legal retrieval or listing systems. This pilot differs from the metadata validation: the unit is not a random source-pool sample but the top ten records visible in a public output stream. The purpose is narrow. It tests whether real upstream legal outputs can be frozen, source-tagged, reconstructed and procedurally classified by the audit layer. It does not evaluate RAG behavior, LLM behavior, legal merits, doctrinal correctness, issue-specific ranking quality or the internal design of the public systems.
...
The framework is not a field deployment study or a new retrieval benchmark. That is by design. It defines the audit artefacts and status rules a deployed or experimental system must satisfy before its outputs receive procedural status. Each validation layer proves a different point: stress scenarios test the decision protocol; metadata packets and public-system outputs test source reconstructability and status allocation; issue-defined gold sets test normative screening against authority and counter-material requirements. None of these layers claims to measure legal merits in the large. They show how legal-output status can be audited above heterogeneous upstream systems.
