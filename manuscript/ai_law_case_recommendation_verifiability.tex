...
The evaluation is not a retrieval leaderboard. It is a falsification design for a procedural-status predicate. The matrix asks four validation questions. First, can the protocol allocate status correctly when the legal-material chain is controlled? Second, does the rule survive public outputs, raw model outputs, source-supported repairs and withheld packets? Third, can simpler substitute theories reproduce the allocation? Fourth, is the allocation stable under policy variation, architecture and model labels, challenge events, certificate replay, tampering and coding uncertainty? This organization separates validation units from generated operations, so the scale of the matrix supports the claim rather than replacing it.
...
\midrule
VQ1 & The status function is decorative or under-specified. & Stress scenarios, formal invariants, status-lattice exhaustion, policy-constants replay and proof-carrying certificates. & Gates, role caps, failure caps and proof obligations reproduce the stated status rule. \\
...
VQ3 & A simpler rule explains the results. & Metric separation, baseline comparison, issue-family holdout, policy-family robustness, gate-contrast witnesses and policy mutation. & Retrieval metrics, total score, source labels, authority coverage, review labels and model identity fail to reproduce the full allocation. \\
...
The complete evaluation matrix adjudicates one rule: procedural status follows the legal-material chain. It contains 264 scenario packets with 697 embedded records or outputs and 7,913,153 validation operations across public-output, model-output, cross-engine, architecture-portability, identity-substitution, issue-family holdout, policy-family robustness, invariant, status-lattice, adversarial, ablation, mutation, review-provenance, claim-anchor, replay, certificate, query, ranking, threat-model coverage and coding-robustness layers. Public-output and holdout layers test real upstream outputs. Model-output and repair layers test whether the same fluent authority recommendations change status when source support, counter-material salience and adoption gates change. Cross-engine, workflow-portability and model-identity layers test the sharper claim: holding the legal-material chain fixed, runtime architecture and provider identity add no procedural status. Metric, baseline, issue-family, policy-family, invariant, lattice, review-provenance and identity-substitution layers test six rival theories--performance sufficiency, source-label sufficiency, authority-material sufficiency, review-label sufficiency, score sufficiency and model-identity sufficiency. Attack, challenge, claim-anchor, replay, mutation and tamper layers test whether any broken link destroys qualification.
...
Authority-material coverage is not procedural qualification & Baseline comparison and status-lattice substitution & High-authority and counter-material coverage reproduce the full screening allocation. \\
...
Substitute-rule failure generalizes across issue families & Issue-family leave-one-out generalization & A substitute rule selected on the remaining issue families reproduces the full protocol on a held-out issue family. \\
Policy choices do not drive the core result & Policy-family robustness & Admissible threshold, salience, source-tag, role-cap and failure-severity variants promote below-screening packets or let simplified substitutes reproduce the protocol. \\
...
The policy is not arbitrary & Policy mutation analysis & Gate-removal or status-conferring mutants survive the committed scenario matrix. \\
Review and adoption labels are not enough & Review-provenance analysis & Completed-review or authorized-adoption labels upgrade incomplete legal-material chains, or missing contestability/adoption records leave high status intact. \\
Validity threats are explicitly covered & Threat-model coverage & A validity threat depends on a single evidence family or lacks an independent role. \\
\bottomrule
\end{tabularx}
\end{table}
