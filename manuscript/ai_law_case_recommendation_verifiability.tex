...
\begin{abstract}
Case recommendation systems decide which legal authorities become visible before legal reasons are written. They are procedural machines, not search accessories. This article introduces \textit{normative material screening output} as the middle category between reference information and decision reasons: a ranked, filtered or summarized authority set that structures later argument without itself becoming law. The framework assigns procedural status through output effect, system-role caps, failure caps and a six-part audit vector covering sources, query trace, authority hierarchy, ranking, counter-materials, contestability and adoption logging. It is implemented as a provider-agnostic harness that emits proof-carrying status certificates. The evaluation combines public retrieval outputs, raw model-output transcripts, source-supported repairs, adversarial attacks, formal invariants, finite-state status-lattice exhaustion, policy replay, certificate tamper-resistance and coding-uncertainty analysis. The result is categorical: procedural status is not reducible to recall, F1, authority coverage, fluent generation, total score, source binding or human-review posture. In the finite high-status lattice, the best partial substitute admitted 672 false positives; the full screening predicate admitted 0 false positives. The contribution is a proof-carrying audit protocol for deciding when legal AI outputs remain reference information, become professional support, qualify as normative material screening, enter decision reasons or lose external legal effect.
\end{abstract}
...
The contributions are three. First, the article names and formalizes normative material screening as a distinct procedural object in AI and Law. Second, it gives that object an explicit status function: output effect, role cap, audit vector, mandatory gates and failure caps jointly decide the maximum procedural status. Third, it supplies an executable validation matrix: 246 scenario packets, 679 embedded records or outputs, and 3,651,560 validation operations across public retrieval, GPT-5.5 transcripts, source-grounding repairs, adversarial source-chain attacks, gate ablations, formal invariants, finite-state status-lattice exhaustion, proof certificates, policy replay and coding-uncertainty analysis. Validation operations include scenario rows, generated counterfactuals, finite-state edges, substitute-rule predictions and replay checks. Its empirical claim is broader than model evaluation: every substitute rule that omits part of the legal-material chain over-admits legal AI outputs.
...
These benchmarks solve the performance question. This article addresses the status question: after an output exists, what procedural position may it claim? COLIEE, LegalBench, CLERC and retrieval benchmarks ask whether a system retrieves, entails, cites or reasons correctly under task conditions. Procedural qualification asks whether the ranked set may be attached to a court report, relied upon in a professional opinion or incorporated into a decision reason. That question turns on corpus records, authority labels, counter-authority handling, contestability and adoption logs.
...
General audit and explainability accounts leave the legal-output problem under-specified. Due-process and public-sector accountability work shows why affected persons need a practical ability to challenge consequential automated support rather than receive a generic explanation after the fact \citep{citron2014,veale2018}. Case recommendation requires a domain-specific account because it structures the visibility and ranking of legal authorities without itself deciding a case. This article supplies that account by treating case recommendation as normative material screening and by specifying the audit artefacts needed to qualify such outputs for procedural use.
...
\item \textbf{Failure consequence: no external legal effect.} This cap applies when core verifiability, contestability or accountability requirements are absent.
\end{enumerate}
...
\]
The predicate $B_D(o)=1$ only when the review gate records completed review, authorized adoption, human authorization, jurisdiction assumptions, adoption reasons and contestation record. Failure flags then apply a cap operator $\kappa_F:\mathcal{P}\rightarrow\mathcal{P}$:
\[
...
The thresholds use non-substitutable gates. The 0--2 scale is ordinal: 0 means absent, 1 means incomplete or internal, and 2 means reviewable by the relevant professional or procedural actor. The default $\tau_N=9$ permits one limited weakness while still requiring every gate; decision-support status requires $T=L=2$ because external reasons require both contestation and adoption records. The full validation suite re-evaluates all 246 packets under thresholds 8--12. Thresholds 8--10 produced identical status allocations; stricter thresholds generated demotions, and no promotions occurred.
...
The complete evaluation matrix tests one rule: procedural status follows the legal-material chain. It contains 246 scenario packets with 679 embedded records or outputs and 3,651,560 validation operations across public-output, model-output, invariant, status-lattice, adversarial, ablation, replay, certificate, query, ranking and coding-robustness layers. The aggregate is a validation-operation count: scenario rows, generated counterfactuals, finite-state edges, substitute-rule predictions and replay checks are reported separately in the artifact. Public-output and holdout layers test reconstruction; model-output layers test source binding and contestability interventions; metric, baseline, invariant and lattice layers test non-equivalence between upstream performance, partial predicates and procedural status; and attack, challenge, replay and tamper layers test whether any missing link breaks qualification.
...
Partial predicates over-admit status & Status-lattice exhaustion & Score, role, source or authority-only predicates reproduce the full high-status allocation. \\
...
Query expansion leaves counter-material protocol necessary & Query-portfolio frontier & Multi-query public retrieval portfolios recover high authority and counter-material sufficiently for screening. \\
...
Status-lattice exhaustion & 233,280 claim-attempt states / 1,632,960 edges & 851/851; 851/851 & Exhausts high-status claim attempts; high-status necessity and gate-ablation drops must hold. \\
\bottomrule
\end{tabularx}
\end{table}
...
Proof-carrying status certificates & 5,658 checks / 4,182 obligations & 5,658/5,658; 4,182/4,182 & Replays proof-carrying status certificates for all 246 packets from scenario identity, scenario hash, policy hash, policy body, jurisdiction profile, score gate, role cap, missing gates, failure cap, metrics, claim support, proof obligations and derivation hash. \\
...
Policy-constants replay & 4,428 checks & 4,428/4,428 & Recomputes score candidates, role caps, protected reliance gates, missing gates, failure caps, metrics and final status in a separate script parameterized by JSON policy constants without importing the harness model. \\
...
The status-lattice layer is a finite-state characterization of high-status claim attempts. It fixes the output claim at decision-support level and enumerates 233,280 score-role-gate states, 1,632,960 cover edges and 1,632,960 substitute-rule predictions. Only 168 states reached high status and only 11 reached decision-support status. All 851/851 high-status necessity checks and 851/851 gate-ablation drops passed. Cover-edge demotions are diagnostics: partial gate additions create incomplete external-use chains, and incomplete chains are supposed to drop. The best partial substitute still admitted 672 false positives; the full screening predicate admitted 0 false positives. This is the formal core of the article: procedural status is a gated legal-material relation, not a scalar score.
...
The harness was also run on ten raw legal-authority recommendations generated by a single upstream model configuration, Codex GPT-5.5 with extra-high reasoning. The prompts covered the same three issue families as the positive-control packets: U.S. agency deference after \textit{Loper Bright}, English mesothelioma causation after \textit{Fairchild}, and EU GDPR Article 15 access rights. The model was not allowed to browse or use tools. This treats the transcript as an upstream legal output and asks whether fluent model text with strong authority coverage can claim normative screening status.
...
The contestation-challenge layer makes the $T$ dimension dynamic. For the same 54 qualified packets, all 216 valid source, authority, jurisdiction and channel challenges blocked high status; all 54 unsupported controls preserved status. Contestability is an operational gate, not a ceremonial opportunity to object.
...
The repair-frontier layer applies the converse test to blocked high-status claims. Across 184 blocked claims and 4,474 counterfactual variants, all 184 were repairable by declared gate families. The minimal-frontier distribution was concentrated: 108 claims required one gate family, 34 required two, 39 required three and three required four. Source-binding, authority-packet and rank-salience completion dominated. Downgrade is therefore diagnostic: the protocol identifies the smallest missing artefact class before reconsideration.
...
The certificate layer makes procedural status proof-carrying. Each packet records identity, hashes, full policy body, profile, scores, role cap, missing gates, failure cap, metrics, 17 proof obligations and a derivation hash. Replaying those certificates verified 246/246 certificates, 5,658/5,658 replay checks and 4,182/4,182 proof obligations. The tamper layer mutated certificate identities, policy bodies, scores, roles, gates, caps, metrics, obligations and certificate-set membership; all 5,415 tamper cases were rejected. A legal AI output can therefore carry its own procedural-status proof object.

The policy-constants replay layer is the independent executable specification. It reads \texttt{policy/legal\_output\_policy.json} for thresholds, status ranks, role caps, protected reliance gates, source-tag classes, jurisdiction-profile aliases and failure dispositions; then it recomputes score candidates, inferred roles, missing gates, metrics, failure flags, failure caps and final status without importing the harness model. It verified 246/246 packets and 4,428/4,428 replay checks. The status allocation must therefore be reproducible both as a proof-carrying certificate and as a second implementation parameterized by the same JSON constants.

The final layers test coding stability. Strict and lenient recoding preserved status for 234/246 packets, with weighted agreement of 1.00 against the strict pass and 0.98 against the lenient pass. Fixed-seed Monte Carlo perturbation of all six audit scores produced 61,500 perturbed evaluations, 0.933 sample-level status stability and 0.924 high-status stability among qualified packets. Score-blinded dual coding of 222 non-holdout packets reached 0.99 inter-coder exact agreement, 0.99 Cohen's kappa and 0.96 quadratic weighted kappa; the weakest dimension-level kappa was 0.93. Base-score calibration was tracked separately: the weakest base-dimension kappa was 0.37 for $Q$ under class imbalance, with 0.96 exact agreement. The coding result supports the same rule as the formal and adversarial layers: status is stable when the legal-material chain is stable, and instability concentrates at the intended threshold gates.
