...
The thresholds use non-substitutable gates. The 0--2 scale is ordinal: 0 means absent, 1 means incomplete or internal, and 2 means reviewable by the relevant professional or procedural actor. The default $\tau_N=9$ permits one limited weakness while still requiring every gate; decision-support status requires $T=L=2$ because external reasons require both contestation and adoption records. The full validation suite re-evaluates all 264 packets under thresholds 8--12. Thresholds 8--10 produced identical status allocations; stricter thresholds generated demotions, and no promotions occurred.
...
\midrule
8 & 10 & 0 & 0 & decision 12; screening 51; professional 23; reference 146; no effect 32 \\
9 & 10 & 0 & 0 & decision 12; screening 51; professional 23; reference 146; no effect 32 \\
10 & 11 & 0 & 0 & decision 12; screening 51; professional 23; reference 146; no effect 32 \\
11 & 12 & 2 & 2 & decision 12; screening 49; professional 25; reference 146; no effect 32 \\
12 & 13 & 58 & 58 & decision 0; screening 17; professional 69; reference 146; no effect 32 \\
\bottomrule
\end{tabularx}
\end{table}
...
The complete evaluation matrix adjudicates one rule: procedural status follows the legal-material chain. It contains 264 scenario packets with 697 embedded records or outputs and 3,658,964 validation operations across public-output, model-output, cross-engine, invariant, status-lattice, adversarial, ablation, replay, certificate, query, ranking and coding-robustness layers. Public-output and holdout layers test real upstream outputs. Model-output and repair layers test whether the same fluent authority recommendations change status when source support, counter-material salience and adoption gates change. Cross-engine layers test the sharper claim: model identity adds no procedural status. Metric, baseline, invariant and lattice layers test four rival theories--performance sufficiency, source-label sufficiency, review-label sufficiency and score sufficiency. Attack, challenge, replay and tamper layers test whether any broken link destroys qualification.
...
Baseline rule comparison & 2,988 predictions & Best simplified false positives 38; reference rule false positives 0 & Tests whether recall, F1, total-score, source-bound or review-gate substitutes can reproduce the full audit allocation. \\
...
Blocked-claim repair frontiers & 4,831 counterfactuals & 193/193 & Computes minimal artifact-gate families needed to upgrade blocked normative-screening or decision-support claims. \\
...
Annotation robustness & 528 recodings & 252/264 stable & Strict and lenient recoding policies test status stability across all packets. \\
Annotation uncertainty & 66,000 perturbations & 0.930 sample stability; 0.916 qualified high-status stability & Fixed-seed score-noise perturbations locate threshold-boundary cases and test coding-noise robustness. \\
...
The cross-engine layer generalizes the same contrast. The same authority-recommendation prompt and issue families were run across Codex GPT-5.5, Codex GPT-5.4-mini and Gemini 2.5 Pro. All nine raw outputs reached perfect issue-set recall and all nine remained reference information because their legal-material chains were not source-bound. The source-supported repair layer then held the issue families constant and supplied manifest-backed source links, locators, procedural source tags, counter-material salience and completed review gates. All nine repaired outputs qualified as normative material screening. Model identity therefore drops out of the status function: the legal-material chain, not the engine label, carries procedural status.
...
The repair-frontier layer applies the converse test to blocked high-status claims. Across 193 blocked claims and 4,831 counterfactual variants, all 193 were repairable by declared gate families. The minimal-frontier distribution was concentrated: 108 claims required one gate family, 40 required two, 42 required three and three required four. Source-binding, authority-packet and rank-salience completion dominated. Downgrade is therefore diagnostic: the protocol identifies the smallest missing artefact class before reconsideration.
...
The final layers test coding stability. Strict and lenient recoding preserved status for 252/264 packets, with weighted agreement of 1.00 against the strict pass and 0.98 against the lenient pass. Fixed-seed Monte Carlo perturbation of all six audit scores produced 66,000 perturbed evaluations, 0.930 sample-level status stability and 0.916 high-status stability among qualified packets. Score-blinded dual coding of 240 non-holdout packets reached 0.99 inter-coder exact agreement, 0.99 Cohen's kappa and 0.97 quadratic weighted kappa; the weakest dimension-level kappa was 0.93. Base-score calibration was tracked separately: the weakest base-dimension kappa was 0.37 for $Q$ under class imbalance, with 0.97 exact agreement. The coding result supports the same rule as the formal and adversarial layers: status is stable when the legal-material chain is stable, and instability concentrates at the intended threshold gates.
