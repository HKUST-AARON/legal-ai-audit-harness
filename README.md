...
| Gate-contrast witness pairs | 336 | Preserves audit scores, upstream metrics and system role while flipping one mandatory evidence, source-tag, authority, counter-material, jurisdiction, contestability or adoption gate; current result is 336/336 status separation. |
| Qualified-output source-chain attacks | 1674 | Mutates every nonempty combination of locator, output-source link, procedural source tag, high-authority recall and counter-material recall attacks across all qualified packets; current result is 1674/1674 loss of high status through downgrade, suspension or withdrawal. |
...
| Proof-carrying status certificates | 9840 | Generates proof-carrying status certificates for all 246 packets and replays scenario identity, scenario hash, policy hash, policy body, jurisdiction profile, score gate, role cap, missing gates, failure cap, metric bundle, claim support, proof obligations and derivation hash. |
| Certificate tamper-resistance | 5415 | Mutates certificate identities, hashes, policy bodies, scores, roles, gates, status fields, caps, metrics, proof obligations and certificate-set structure; current result is 5415/5415 rejected. |
| Policy-constants replay | 4428 | Recomputes score candidates, role caps, protected reliance gates, missing gates, failure caps, metrics and final status in a separate script parameterized by `policy/legal_output_policy.json` constants without importing the harness model; current result is 4428/4428 passed. |
