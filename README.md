...
An executable scoring harness for legal AI outputs above any search, retrieval, generation, agent, database, or manual review stack.
...
python scripts/run_status_lattice_analysis.py
python scripts/run_metric_separation_analysis.py
python scripts/run_ranked_retrieval_metric_analysis.py
python scripts/run_baseline_comparison_analysis.py
python scripts/run_issue_family_generalization.py
python scripts/run_multiaxis_generalization.py
python scripts/run_gate_ablation_analysis.py
python scripts/run_gate_contrast_witness_analysis.py
...
python scripts/run_certificate_tamper_analysis.py
...
python scripts/run_temporal_validity_analysis.py
python scripts/run_workflow_portability_analysis.py
...
```json
{
  ...
  "review_gate": {
    ...
    "contestability_channel": "review-record",
...
| Cross-engine raw model outputs | 9 | Tests the same issue prompts across three model engines; all high-recall raw outputs remain reference information without source-bound legal-material chains. |
| Cross-engine source-supported repairs | 9 | Tests the same cross-engine outputs after source-support validation; all qualify as normative material screening. |
...
| Ranked-retrieval metric analysis | 3806 | Computes nDCG, MRR, MAP, precision@5 and recall@k over 258 ranked packets; current result is best single-metric FP/FN 135/2 and high-nDCG@10 blocked 160/223. |
...
| Issue-family leave-one-out generalization | 3014 | Holds out each of 5 issue families, selects simplified substitute rules on the remaining families, and tests holdout false positives against the full protocol. |
| Multi-axis holdout generalization | 110568 | Holds out issue family, validation suite, system role, jurisdiction profile, claimed status and output-origin strata; current result is full protocol FP/FN 0/0, best trained simplified FP 510 and best-rule errors in 17/34 folds. |
| Policy-family robustness | 42192 | Varies thresholds, rank-salience windows, procedural source tags, role caps and failure severity across 12 policy variants; no variant promotes a below-screening packet to high status. |
| Substitute-theory falsification | 7 theories | Reports false-positive and false-negative counts for performance sufficiency, source-label sufficiency, authority-material sufficiency, review-label sufficiency, score sufficiency, model-identity sufficiency and ranked-retrieval metric sufficiency; current full-protocol false positives and false negatives are 0. |
| Qualified-output gate ablations | 390 | Removes procedural gates from all qualified packets and checks that status falls below the corresponding procedural level. |
| Gate-contrast witness pairs | 390 | Preserves audit scores, upstream metrics and system role while flipping one mandatory evidence, source-tag, authority, counter-material, jurisdiction, contestability or adoption gate; current result is 390/390 status separation. |
| Qualified-output source-chain attacks | 1953 | Mutates every nonempty combination of locator, output-source link, procedural source tag, high-authority recall and counter-material recall attacks across all qualified packets; current result is 1953/1953 loss of high status through downgrade, suspension or withdrawal. |
| Dynamic contestation challenges | 315 | Applies valid counter-material, source-verification, jurisdiction and contestability-channel challenges plus unsupported challenge controls; current result is 252/252 valid challenges blocked and 63/63 unsupported controls preserved. |
| Blocked-claim repair frontiers | 5097 | Computes the minimal artifact-gate families needed to upgrade each blocked normative-screening or decision-support claim. |
| Jurisdiction-profile mutations | 440 | Checks high-status profile support and mutates qualified packets to test missing or mismatched jurisdiction assumptions. |
| Ranking-visibility diagnostics | 956 | Computes a rank-window visibility curve over 248 high-status claims and runs 85 rank-order counterfactuals where the packet can move counter-material below the visibility window. |
| Proof-carrying status certificates | 10560 | Generates proof-carrying status certificates for all 264 packets and replays scenario identity, scenario hash, policy hash, policy body, jurisdiction profile, score gate, role cap, missing gates, failure cap, metric bundle, claim support, proof obligations and derivation hash. |
| Certificate tamper-resistance | 5811 | Mutates certificate identities, hashes, policy bodies, scores, roles, gates, status fields, caps, metrics, proof obligations and certificate-set structure; current result is 5811/5811 rejected. |
| Policy-constants replay | 4752 | Recomputes score candidates, role caps, protected reliance gates, missing gates, failure caps, metrics and final status in a separate script parameterized by `policy/legal_output_policy.json` constants without importing the harness model; current result is 4752/4752 passed. |
...
| Temporal source-validity analysis | 189 | Mutates qualified packets with stale source snapshots, post-snapshot authority-status changes and refreshed temporal controls; current result is 189/189 passed. |
| Workflow portability analysis | 2904 | Tests architecture-label invariance, deployment entitlement caps, decision dependency on screening-capable chains and unaccountable external-use bars across all scenario packets. |
...
| Construct-operationalization coverage | 42 | Maps 10 core paper constructs to committed evidence layers; current result is 42/42 checks passed across 33 evidence layers. |
| Threat-model coverage | 34 | Maps 8 validity threats to 32 committed evidence layers; current result is 34/34 coverage checks passed. |
...
| Annotation robustness recoding | 528 | Re-scores all 264 scenario packets under strict and lenient coding policies to test status stability. |
| Annotation uncertainty Monte Carlo | 66000 | Perturbs all six audit scores under a fixed-seed noise model to locate threshold-boundary cases and test status stability under plausible coding uncertainty. |
...
The current full validation report covers 264 scenario files containing 697 embedded records/items, 30 public source-text anchor checks, 50 raw model-output transcript locator checks, 36 cross-engine transcript locator checks, 51,648 formal invariant checks, 466,560 high-status claim-attempt states, 3,499,200 status-lattice cover edges, 3,732,480 status-lattice substitute-rule predictions, 219 metric-separation evaluations, 2,000 metric statistical resamples, 1,806 ranked-retrieval metric predictions, 2,000 ranked-metric resamples, 3,252 baseline-rule predictions, 3,014 issue-family holdout predictions, 110,568 multi-axis generalization predictions, 42,192 policy-family robustness evaluations, 390 gate-ablation evaluations, 390 gate-contrast witness pairs, 1,953 source-chain attack variants, 315 contestation challenge variants, 1,233 metamorphic policy tests, 3,111 policy-mutation evaluations, 627 review-provenance checks, 1,080 claim-anchor mutations, 189 temporal source-validity mutations, 2,904 workflow-portability checks, 1,320 model-identity substitutions, 30 query-perturbation variants across 5 issue groups, 315 query portfolios plus 5 group frontier summaries, 42 construct-operationalization checks across 10 core constructs, 5,097 repair-frontier evaluations, 440 jurisdiction-profile checks, 956 rank-window visibility checks, 85 rank-order counterfactuals, 6,072 status-certificate replay checks, 4,488 certificate proof obligations, 5,811 certificate tamper-resistance cases and 4,752 policy-constants replay checks, plus 66,000 score-uncertainty perturbations, strict/lenient recoding, score-blinded codebook coding and full-threshold sensitivity checks. The matrix tests a single rule: procedural status follows the reconstructable legal-material chain. Its substitute-theory table falsifies performance sufficiency, source-label sufficiency, authority-material sufficiency, review-label sufficiency, score sufficiency, model-identity sufficiency and ranked-retrieval metric sufficiency; the full protocol records 0 false positives and 0 false negatives against those substitutes. Public retrieval, holdout, model-output, cross-engine, source-supported repair, evidence ladder, public source-text anchors, transcript anchors, formal invariants, status-lattice exhaustion, metric separation, ranked retrieval metrics, baseline comparison, issue-family generalization, multi-axis generalization, policy-family robustness, gate ablation, gate-contrast witnesses, source-chain attacks, contestation challenges, temporal source-validity mutations, proof-carrying certificates, certificate tamper-resistance, policy-constants replay, metamorphic policy checks, policy-mutation killing, review-provenance falsification, claim-anchor falsification, workflow portability, model-identity invariance, query perturbation, query portfolios, construct-operationalization coverage, repair frontiers, jurisdiction profiles, ranking visibility, adversarial repairs, annotation uncertainty, threshold sensitivity, robustness and blind coding all test whether that chain survives the conditions required for procedural use.

The multi-axis holdout generalization layer adds 19,235 holdout predictions and 91,333 training predictions across 34 folds on 6 axes. It holds out issue family, validation suite, system role, jurisdiction profile, claimed status and output-origin strata; the full protocol records 0 false positives and 0 false negatives, while the best trained simplified substitute produces 510 holdout false positives and errs in 17/34 folds.

The ranked-retrieval metric layer can also be run directly:

```bash
python scripts/run_ranked_retrieval_metric_analysis.py
```

It computes ranked-retrieval metrics over 258 ranked packets. The current run records 1,806 threshold predictions over 7 legal-IR metrics, with best single-metric FP/FN 135/2 and 160/223 high-nDCG@10 outputs blocked below normative screening.

The construct-operationalization layer adds 42 checks across 10 core paper constructs, linking status ordering, role caps, audit-vector coding, source-chain evidence, authority hierarchy, counter-material salience, contestability/adoption, substitute-theory separation, portability and replication to committed evidence layers.

The threat-model coverage layer adds 34 threat-model coverage checks across 8 validity threats, linking construct, internal, source-chain, external, architecture, policy, provenance and annotation validity to independent evidence layers.
...
The temporal source-validity layer can also be run directly:

```bash
python scripts/run_temporal_validity_analysis.py
```

The workflow-portability layer can also be run directly:

```bash
python scripts/run_workflow_portability_analysis.py
```

The policy-family robustness layer can also be run directly:

```bash
python scripts/run_policy_family_robustness.py
```

The multi-axis holdout generalization layer can also be run directly:
...
```bash
python scripts/run_threat_model_coverage_analysis.py
```
...
By default the script uses the committed public metadata snapshots for deterministic reruns. Pass `--refresh` to download fresh snapshots. The experiment samples 20 records per jurisdiction with a fixed seed, writes source manifests, and runs the harness over the generated output evidence packets. It tests whether evidence packets can be reconstructed and audited. These metadata-only packets are capped at `professional_support_output` because they supply source reconstruction without issue-specific counter-material gold sets.
...
The sensitivity command varies the normative-screening threshold from 8 to 11 for a selected scenario directory. The full validation suite additionally re-evaluates all 264 scenario packets under thresholds 8-12, so threshold robustness is tested across the complete artifact rather than only the stress scenarios.
...
The packet builder strips original `scores`, `expected_allowed_status`, `expected_disposition`, source paths, and manual failure flags from 240 committed non-holdout score-blinded coding packets. The resulting packet files preserve the legal-output evidence needed for coding: claimed status, jurisdiction profile, authority sets, upstream metrics, evidence packet, review gate, deployment context and packet identifiers. Two separate annotation files in `experiments/blind_coding/annotations/` then score the packets under the shared codebook in `experiments/blind_coding/CODEBOOK.md`. The study reports coder-coder status kappa, base-coder status agreement against the harness allocation, inter-coder dimension-level kappa, post-evaluation failure-flag agreement, post-evaluation missing-gate agreement, base-dimension calibration and disputed packets. Current results are inter-coder minimum dimension kappa 0.93, minimum derived failure-flag exact agreement 0.97, minimum derived missing-gate exact agreement 0.98, weakest base-dimension kappa 0.37 on Q, base-dimension minimum exact agreement 0.87, minimum three-category PABAK 0.81 and maximum mean absolute score drift 0.13.
...
The builder validates each repaired output-source link against the issue manifest, locator, hashed manifest source-support excerpt, contradiction patterns, procedural source tag and high-authority/counter-material coverage before writing `source_binding_validation` and the derived audit-vector scores. The current result is the paired contrast used in the paper: all ten raw outputs remain `reference_information`; all ten manifest-backed validator repairs qualify as `normative_material_screening_output`.

The cross-engine layer repeats the authority-recommendation test across Codex GPT-5.5, Codex GPT-5.4-mini and Gemini 2.5 Pro:

```bash
python scripts/build_cross_engine_model_outputs.py
python scripts/verify_cross_engine_model_transcripts.py
python -m audit_harness.cli experiment experiments/cross_engine_model_outputs/scenarios --out experiments/cross_engine_model_outputs/results/cross_engine_model_output_experiment.md --json-out experiments/cross_engine_model_outputs/results/cross_engine_model_output_experiment.json
python -m audit_harness.cli experiment experiments/cross_engine_model_repairs/scenarios --out experiments/cross_engine_model_repairs/results/cross_engine_model_repair_experiment.md --json-out experiments/cross_engine_model_repairs/results/cross_engine_model_repair_experiment.json
```

The result is the identity-neutral claim in executable form: 9/9 raw cross-engine outputs are capped at `reference_information` despite perfect authority recall, 9/9 source-supported repairs qualify as `normative_material_screening_output`, and 36/36 transcript locators are anchored in committed raw transcripts. Model identity is not a status-conferring property.
...
The benchmark freezes 22 public search outputs across five issue families: U.S. agency deference after `Loper Bright`, English mesothelioma causation after `Fairchild`, Canadian administrative-law standard of review after `Vavilov`, German/EU right-to-be-forgotten review, and EU GDPR Article 15 access rights. It uses committed CourtListener, National Archives, Supreme Court of Canada Lexum, OpenLegalData and CURIA snapshots, then measures high-authority recall, counter-material recall, source reconstruction and procedural status. The gold sets are matched to endpoint scope; mixed statute/case screening is tested in the source-screening packets. The current result is stringent: all 22 real search outputs remain `reference_information`, with mean high-authority recall of 0.23 and mean counter-material recall of 0.00, because top-k public search results do not preserve the complete issue-specific authority and counter-material chain.
...
This pilot freezes the ordered top 10 records exposed by each of six public legal retrieval or listing systems and converts those visible outputs into provider-agnostic evidence packets. It tests whether real public system outputs can be reconstructed, source-tagged and status-qualified by the harness. The current public-output scenarios are capped at `professional_support_output` because they lack issue-defined counter-material packets and party-facing contestation pathways.
...
experiments/multiaxis_generalization/ cross-stratum holdout generalization results
