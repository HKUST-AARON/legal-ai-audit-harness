...
| Proof-carrying status certificates | 9348 | Generates proof-carrying status certificates for all 246 packets and replays scenario identity, scenario hash, policy hash, policy body, jurisdiction profile, score gate, role cap, missing gates, failure cap, metric bundle, claim support, proof obligations and derivation hash. |
...
| Policy-constants replay | 4182 | Recomputes score candidates, role caps, missing gates, failure caps, metrics and final status in a separate script parameterized by `policy/legal_output_policy.json` constants without importing the harness model; current result is 4182/4182 passed. |
